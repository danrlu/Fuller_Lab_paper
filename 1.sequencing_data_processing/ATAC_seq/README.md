...
#### 7.2 create .bigwig
convert .bed to .bedgraph which is needed to convert to .bigwig for genomic viewer viewing. during conversion to .bedgraph, each library is normalized by total read counts (`-scale` is calculated by 10,000,000/total reads, practically reads per 10million or 10x RPM...). Note the reads used at this step is already filtered by deplicate, MAPQ and on main chromasomes. 
```bash
